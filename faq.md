...
1. **Enable optimizations**: For TorchScript models, use `optimize_for_inference`:

```
module = torch::jit::optimize_for_inference(module);
```

1. **Warm up the model**: Run a few inference passes before benchmarking to allow
JIT compilation and memory allocation to complete.
2. **Check thread settings**: Ensure proper thread configuration:
